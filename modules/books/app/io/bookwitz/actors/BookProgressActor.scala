...
/**
  ...
  */
class BookProgressActor(file: File, user: BasicUser, title: String) extends Actor with akka.actor.ActorLogging {
  ...
  def receive = {
    ...
    case "start" => {
      processActor = BookController.system.actorOf(Props(new BookProcessActor(file, user, title)))
      processActor ! 0
      status = "Processing."
    }
  }
